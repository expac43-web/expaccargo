...
import {
  LayoutDashboard, Users, Package, FolderOpen, FolderLock, MessagesSquare,
...
const navItems: NavItem[] = [
  ...
  { label: "Mes documents",   href: "/dashboard/agence/mes-documents", icon: FolderLock },
  { label: "Messagerie",      href: "/dashboard/agence/messagerie",   icon: MessagesSquare },
  { label: "Mon profil",      href: "/dashboard/agence/profil",       icon: UserCircle },
];

const BG = "linear-gradient(165deg, #0c3d4a 0%, #0e5f72 60%, #0d7a8f 100%)";
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="h-16 flex items-center justify-between px-5 border-b border-white/10 shrink-0">
        ...
        <div className="flex items-center gap-1">
          <GlobalSearch clientBase="/dashboard/agence/clients" tone="dark" />
          <NotificationBell tone="dark" accent="#0e5f72" />
          {onClose && (
      ...
      <div className="px-4 py-4 border-b border-white/10 shrink-0">
        <div className="flex items-center gap-3 px-3 py-3 rounded-xl bg-white/10">
          <div className="w-9 h-9 rounded-xl overflow-hidden flex items-center justify-center text-[#0c3d4a] text-sm font-black shrink-0 bg-white relative">
            {avatarUrl ? (
          ...
          <div className="min-w-0">
            ...
            <p className="text-white/50 text-xs truncate" style={{ fontFamily: "var(--font-lato)" }}>{agencyName || "Agence"}</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        <div className="pt-1 pb-2 px-3">
          <p className="text-xs font-black uppercase tracking-widest text-white/30" style={{ fontFamily: "var(--font-montserrat)" }}>Espace agence</p>
        </div>

        {navItems.map(({ label, href, icon: Icon }) => {
          const badge = label === "Messages" ? unread : 0;
          const active = href === "/dashboard/agence"
          ...
          return (
            ...
            >
              ...
              {!!badge && (
                <span className="w-5 h-5 rounded-full flex items-center justify-center text-xs font-black" style={{ backgroundColor: "#0d9488", color: "#fff", fontFamily: "var(--font-montserrat)" }}>
                  {badge > 9 ? "9+" : badge}
                </span>
              )}
              {active && (
                <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-r-full" style={{ backgroundColor: "#2dd4bf" }} />
              )}
            </Link>
          );
        })}
...
export default function AgenceSidebar({ userName, agencyName, avatarUrl = null }: { userName: string; agencyName: string; avatarUrl?: string | null }) {
  ...
  useEffect(() => {
    async function poll() {
      try {
        const r = await fetch("/api/agence/messages", { method: "HEAD" });
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-14 z-40 flex items-center px-4 gap-3 shadow-lg" style={{ background: BG }}>
        ...
        <div className="ml-auto flex items-center gap-2">
          {unread > 0 && (
            <Link href="/dashboard/agence/messages" className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-black text-white" style={{ backgroundColor: "#0d9488", fontFamily: "var(--font-montserrat)" }}>
      ...
      <aside className="hidden lg:flex w-60 shrink-0 flex-col min-h-screen sticky top-0 h-screen shadow-xl z-30" style={{ background: BG }}>
        <NavContent userName={userName} agencyName={agencyName} avatarUrl={avatarUrl} pathname={pathname} unread={unread} />
      </aside>
      ...
      {open && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-64 flex flex-col shadow-2xl" style={{ background: BG }}>
            <NavContent userName={userName} agencyName={agencyName} avatarUrl={avatarUrl} pathname={pathname} unread={unread} onClose={() => setOpen(false)} />
          </div>
        </div>
      )}
    </>
