...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Le manager peut voir toutes les conversations
  const [conv] = await sbGet<{ id: string; clientId: string; agencyId: string | null; type: string }>(
    "Conversation", `id=eq.${enc(id)}&select=id,clientId,agencyId,type&limit=1`
  );
  ...
  // Filtre de suppression côté requête : le manager voit
  // - ses propres messages non supprimés par lui (émetteur ou destinataire)
  // - les échanges client↔agence tant qu'ils ne sont pas supprimés des deux côtés
  const me = enc(user.id);
  const delFilter = `or=(and(senderId.eq.${me},deletedBySender.eq.false),and(receiverId.eq.${me},deletedByReceiver.eq.false),and(senderId.neq.${me},receiverId.neq.${me},or(deletedBySender.eq.false,deletedByReceiver.eq.false)))`;
  ...
  // Marquer comme lus uniquement les messages adressés à ce manager
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // body: content + asAgency (true = répond au nom de l'agence dans une conv CLIENT_AGENCY)
  const { content, asAgency } = await req.json();
  ...
  const msg = await sbPost<Msg>("Message", {
    ...
    // sentAsManager=false quand il répond au nom de l'agence ; true pour conv directe manager
    sentAsManager: !asAgency,
    createdAt: now,
  });
