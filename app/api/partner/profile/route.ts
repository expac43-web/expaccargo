...
/** Profil du compte partenaire connecté (mêmes règles que les profils staff). */
export async function GET() {
  const session = await auth();
  const userId = (session?.user as { id?: string })?.id;
  const role = (session?.user as { role?: string })?.role;
  if (!session || role !== "PARTNER" || !userId) {
    return NextResponse.json({ error: "Non autorisé" }, { status: 401 });
  }

  const users = await sbGet<{
    id: string; name: string; email: string; phone: string | null; whatsapp: string | null;
  }>("User", `id=eq.${enc(userId)}&select=id,name,email,phone,whatsapp`);
...
export async function PATCH(req: NextRequest) {
  ...
  if (newPassword) {
    ...
    const users = await sbGet<{ password: string }>("User", `id=eq.${enc(userId)}&select=password`);
    const valid = users[0] && (await bcrypt.compare(currentPassword, users[0].password));
    if (!valid) return NextResponse.json({ error: "Mot de passe actuel incorrect." }, { status: 400 });
    updates.password = await bcrypt.hash(newPassword, 12);
  }

  await sbPatch("User", `id=eq.${enc(userId)}`, updates);
