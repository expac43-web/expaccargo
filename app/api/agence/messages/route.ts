...
export async function GET(req: NextRequest) {
  ...
  // Messages envoyés par cet agent ou reçus par lui
  let query = `or=(senderId.eq.${userId},receiverId.eq.${userId})&select=*&order=createdAt.asc`;
  if (clientId) {
    query = `or=(and(senderId.eq.${userId},receiverId.eq.${clientId}),and(senderId.eq.${clientId},receiverId.eq.${userId}))&select=*&order=createdAt.asc`;
  }
  if (shipmentId) query += `&shipmentId=eq.${shipmentId}`;
...
export async function HEAD(req: NextRequest) {
  ...
  const role = (session?.user as { role?: string })?.role;
  const userId = (session?.user as { id?: string })?.id;
  if (!session || !isAgency(role) || !userId) return new NextResponse(null, { status: 401 });

  const unread = await sbGet<{ id: string }>(
    "Message",
    `receiverId=eq.${userId}&isRead=eq.false&select=id`
  );
...
export async function POST(req: NextRequest) {
  ...
  const { receiverId, content, shipmentId } = await req.json();
