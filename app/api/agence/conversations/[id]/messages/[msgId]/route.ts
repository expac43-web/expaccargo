...
export async function DELETE(
  ...
  const [conv] = await sbGet<{ id: string }>(
    "Conversation", `id=eq.${enc(id)}&agencyId=eq.${enc(user.agencyId)}&select=id&limit=1`
  );
  ...
  // Non lu et expéditeur : supprimer pour tout le monde
