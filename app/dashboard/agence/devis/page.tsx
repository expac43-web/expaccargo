...
export default function AgenceDevisPage() {
  ...
  async function downloadPdf(q: Quote) {
    ...
    exportDevisPDF({
      ...
      transportMode: q.transportMode, preferredDate: q.preferredDate,
      quotedPrice: q.quotedPrice, quotedCurrency: q.quotedCurrency,
      quoteMessage: q.quoteMessage, quoteItems: q.quoteItems,
                    ...
                    <div className="min-w-0">
                      ...
                      {q.handledByName && (
                        <p className="text-xs text-gray-500 mt-1" style={{ fontFamily: "var(--font-lato)" }}>
                          Traité par <strong>{q.handledByName}</strong>
                        </p>
                      )}
                      {q.signature && (
