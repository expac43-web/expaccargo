...
export default function GerantDocumentsPage() {
  ...
  async function loadDocs() {
    const url = filterClient ? `/api/gerant/documents?clientId=${filterClient}` : "/api/gerant/documents";
    const r = await fetch(url);
    setDocs(r.ok ? await r.json() : []);
  }

  useEffect(() => {
    fetch("/api/gerant/clients").then((r) => r.ok ? r.json() : []).then(setClients);
  }, []);
  ...
  async function handleUpload() {
    ...
    const r = await fetch("/api/gerant/documents", { method: "POST", body: fd });
    setUploading(false);
    if (!r.ok) { const d = await r.json(); setUploadError(d.error ?? "Erreur upload."); return; }
    setUploadSuccess(true);
    setUploadFile(null); setUploadClient(""); setUploadType("OTHER");
    await loadDocs();
  ...
  async function handleDelete(id: string) {
    ...
    await fetch(`/api/gerant/documents/${id}`, { method: "DELETE" });
    ...
    <div className="p-6 lg:p-8 max-w-7xl">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        ...
        <select value={filterClient} onChange={(e) => setFilterClient(e.target.value)}
          className="px-3 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-[#1A3A6B] bg-white"
          style={{ fontFamily: "var(--font-lato)" }}>
      ...
      {loading ? (
        <div className="space-y-3">{[...Array(4)].map((_, i) => <div key={i} className="h-16 bg-white rounded-xl border border-gray-100 animate-pulse" />)}</div>
      ) : filtered.length === 0 ? (
              ...
              <div key={doc.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <a href={`/api/files/${doc.id}?download=1`} title="Télécharger"
                    className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-[#1A3A6B] hover:bg-gray-100 transition-all">
                    <Download size={15} />
                  </a>
                  {doc.canDelete && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 z-10">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setShowModal(false)} className="w-7 h-7 flex items-center justify-center rounded-lg bg-gray-100 text-gray-500"><X size={14} /></button>
            </div>
            {uploadError && (
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-black uppercase tracking-wider mb-1.5 text-gray-600" style={{ fontFamily: "var(--font-montserrat)" }}>Type</label>
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center cursor-pointer hover:border-[#1A3A6B] transition-colors"
                  onClick={() => fileRef.current?.click()}>
                  <Upload size={24} className="mx-auto text-gray-300 mb-2" />
                  {uploadFile
                    ? <p className="text-sm text-gray-700 font-medium" style={{ fontFamily: "var(--font-lato)" }}>{uploadFile.name}</p>
                    : <p className="text-sm text-gray-400" style={{ fontFamily: "var(--font-lato)" }}>Cliquez pour sélectionner (max 20 Mo)</p>}
